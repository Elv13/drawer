...
#echo "Core: $CORE"
    ...
    USAGE=$(cat /proc/cpuinfo | grep "cpu MHz" | cut -d":" -f 2 | sed -n $(printf "%sp" $(($CORE+1))))
    #echo "Usage: $USAGE"
	...
	CORE_INFO[$CORE]="usage =\"$USAGE\", iowait = \"$IOWAIT\", idle = \"$IDLE\""
...
for CURRENT_CORE in `seq 0 $CORE_NB`;do
    ...
    echo "   core${CURRENT_CORE} = { speed= \"${CORE_ONE}\", temp= \"${TEMP_ONE:4}C\", ${CORE_INFO[${CURRENT_CORE}]}},"
done
echo "}"
